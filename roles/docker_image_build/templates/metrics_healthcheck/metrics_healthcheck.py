    ...
    if  earliest_overall_age > allowed_age:
        ...
        if latest_group_series_timestamp:
            ...
            if latest_group_series_age > allowed_age:
                print("bad ({0}/{1} is {2} seconds old)".format(group, series, latest_group_series_age))
                exit(-1)
            else:
                print("good ({0}/{1} is {2} seconds old)".format(group, series, latest_group_series_age))
                exit(0)
        else:
            print("bad (no logs, {0}/{1} is at least {2} seconds stale)".format(group, series, earliest_overall_age))
            exit(-1)
    else:
